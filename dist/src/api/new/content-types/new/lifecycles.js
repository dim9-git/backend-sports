...
exports.default = {
    async beforeCreate(event) {
        ...
        if (!data.slug) {
            try {
                event.params.data.slug = await generateUniqueSlug(data.title, data.locale, 'api::new.new');
            }
            catch (error) {
    ...
    async beforeUpdate(event) {
        ...
        if (data.title && !data.slug) {
            try {
                ...
                event.params.data.slug = await generateUniqueSlug(data.title, data.locale, 'api::new.new', existingEntry === null || existingEntry === void 0 ? void 0 : existingEntry.slug);
            }
            catch (error) {
                console.error('Error generating slug:', error);
                throw error;
